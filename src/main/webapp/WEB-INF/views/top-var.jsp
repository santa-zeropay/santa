<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Top-area</title>
<link rel="stylesheet" href="/resources/css/top-var.css">
</head>
<body>
	<div class="top-area">
		<i class="fas fa-bars fa-2x"></i>
		<div class="list">W.P</div>
		<a href="/main" class="list">가게 리스트</a> <a href="/" class="list">지도로
			보기</a> <a href="/recommend" class="list">추천 패키지</a> <a
			href="/user/myPage" class="list">내 정보</a> <a href="/store/myStore"
			class="list">내 가게</a>
		<div class="status-var">
			<div>
				<input type="text"> <i class="fas fa-search fa-2x"></i>
			</div>
			<a href="/user/cart"><i class="fas fa-shopping-cart fa-2x "></i></a>
		</div>

	</div>
</body>
</html>